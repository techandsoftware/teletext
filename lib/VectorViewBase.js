...
export class VectorViewBase {
    ...
    _update() {
        ...
        this._gridrows.forEach((rowView, rowIndex) => {
            ...
            this._defs.transferToClipPath_(this._boxLayer, rowIndex);
        });
    ...
    _resetBoxClipForRow(rowNum) {
        this._boxLayer.removeChildrenForRow_(rowNum);
    }
    ...
    _setBoxDoubleHeight() {
        this._defs.rects_().forEach(box => box.height_(CELL_DOUBLE_HEIGHT));
    }

    _setBoxForRow(rowNum, colNum) {
        const x = colNum * CELL_WIDTH;
        const y = rowNum * CELL_HEIGHT;
        this._lastBoxBuffer = this._defs.rect_(CELL_WIDTH, CELL_HEIGHT).move_(x, y);
    }

...
const BOX = {
    START: Attributes.charFromAttribute(Attributes.START_BOX),
    END: Attributes.charFromAttribute(Attributes.END_BOX)
};
...
export class PageModel {
    ...
    getRow_(rowNum) {
        ...
        this._screen[rowNum].forEach((cell, cellIndex) => {
            ...
            const nextByte = cellIndex + 1 < CELLS_PER_ROW ?
                this._screen[rowNum][cellIndex + 1].byte_
                : null;
...
const attributeHandlers = {
    ...
    [Attributes.START_BOX]: ({ rs, cell, previousByte }) => {
        if (previousByte == BOX.START) {
            cell.boxed_ = true;
            rs.nextBoxed = true;
        }
    },
    // set between two end box attributes
    [Attributes.END_BOX]: ({ rs, nextByte }) => {
        if (nextByte == BOX.END) {
            rs.nextBoxed = false;
        }
    },

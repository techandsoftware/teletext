...
class Defs extends Element {
    ...
    transferToClipPath_(clipPath, rowNum) {
        this._c.forEach(rect => {
            rect.data_('r', rowNum);
            clipPath._e.appendChild(rect._node());
            clipPath._c.push(rect);
        });
        this._c = [];
    }
}
...
class ClipPath extends Element {
    ...
    removeChildrenForRow_(rowNum) {
        this._c = this._c.filter(rect => {
            if (rect.data_('r') == rowNum) {
                rect.remove_();
                return false;
            }
            return true;
